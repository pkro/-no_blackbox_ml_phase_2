...
featureFunctions.getElongation = (paths)=> {
   ...
   // add 1 because it could be 0 and give an error, e.g. for a perfectly straight line
...
featureFunctions.getPixels = (paths, size=400, expand=true)=>{
   ...
   // expand to full canvas size (basically crop and scale up to full canvas size)
   // reasoning: small drawings can be as complex as larger drawings, but smaller
   // drawings are penalized (less pixels) if we don't upscale
   if (expand) {
      const points = paths.flat();
   ...
   // take every 4th value (alpha channel) as lines are white and bg is black
...
featureFunctions.getComplexity = (paths) => {
   ...
   const complexity = pixels.filter((val) => val > 0).length;
   return complexity;
